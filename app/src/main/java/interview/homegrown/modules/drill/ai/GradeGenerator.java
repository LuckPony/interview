...
public class GradeGenerator {
    ...
    public GradeOutput grade(String stem, String rawAnswer, List<ConceptPointGroup> groups,
        ...
        if (conversation == null || conversation.isBlank()) {
            ...
            conversationBlock = "\n\n对话实录（老师-学生逐轮问答，按时间顺序，学生消息标注轮次与「独立作答/已给引导提示后」）。"
                    + "评分依据：\n"
                    + "1) 每个评分点只能依据「用户答案」字段（这是被提示前的独立作答）以及实录中标注「独立作答」的学生消息来判 HIT/PARTIAL/MISS；\n"
                    + "2) 标注「已给引导提示后」的学生作答是学生看到 AI 引导提示后的补全，不代表独立掌握，一律不得作为判 HIT 的依据（只能作参考）；\n"
                    + "3) 若某评分点老师从未问过、学生也从未在独立作答中触及 → 判 NA（不计分，不算 MISS）；\n"
                    + "4) 老师是否已泄底：若老师消息已给出某评分点的答案，学生其后任何照抄该点的回答都判 NA，不得给 HIT；\n"
                    + "5) 学生重复他人建议、答非所问的内容不得给 HIT。\n"
                    + "综合判定每个评分点：\n"
                    + conversation;
        }
