...
public class GradeGenerator {
    ...
    public GradeOutput grade(String stem, String rawAnswer, List<ConceptPointGroup> groups,
        ...
        if (conversation == null || conversation.isBlank()) {
            ...
            conversationBlock = "\n\n对话实录（老师-学生逐轮问答，按时间顺序，学生消息标注轮次）。"
                    + "用于核验：1) 学生最终完整作答覆盖了哪些评分点（引导后的补全也计入）；"
                    + "2) 哪些评分点老师从未问过/学生从未触及 → 判 NA（不计分，不算 MISS）；"
                    + "3) 老师是否已泄底（若老师消息已给出某评分点的答案，学生其后照抄该点的回答判 NA，不得给 HIT）；"
                    + "4) 学生重复他人建议、答非所问的内容不得给 HIT。"
                    + "综合学生全部作答（以「用户答案」字段中的最终完整作答为主）判定每个评分点：\n"
                    + conversation;
        }
