...
public class QuestionGenerator {
    ...
    public Prompt prompt(SelectedTask task, ProbeType probeType, ResponseFormat format,
        ...
        String system = """
                ...
                题干必须只抛出一个核心问题（一个主问），不要在一个题干里堆砌多个小问、
                不要一段话里塞四五个问句；更深入的追问会在学习者作答后由老师针对其回答逐条进行。
                如果概念是算法 / 数据结构 / 代码实现类：请在题干里指定一道具体的 LeetCode 题目
                （题号 + 题名），并明确提示"先到 LeetCode 完成这道题，然后把通过全部用例的代码
                粘贴到输入框"；复杂度、边界、优化等更深的追问同样留到作答后的对话中逐条进行。
        ...
        String user = String.format("""
                ...
                - 题干只抛出一个核心问题（一个主问）；作答后的更深入追问（最多 4 个小问）
                  会在对话中由老师基于学习者的回答和薄弱点逐条进行，不需要你在题干里预埋小问。
