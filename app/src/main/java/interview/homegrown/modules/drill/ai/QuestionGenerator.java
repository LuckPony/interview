    ...
    public Prompt prompt(SelectedTask task, ProbeType probeType, ResponseFormat format,
        ...
        String system = """
                ...
                - 追问只作为候选，老师必须在学习者完全回答当前问题或明确确认理解后才可能提出，
                  也可以提前停止，一个都不问。
                【动手实现优先】不要把所有题都出成"解释/说明"。若概念本质是"怎么实现/怎么做"
                （算法、数据结构、并发控制、数据库查询与索引、网络协议处理、脚本命令、配置构建、
                框架机制等），优先出"动手实现"题，让学习者直接写出可运行代码 / SQL / 命令 / 配置，
                而不是口头解释：
                - 题干用指令动词开头（"写出""实现""补全""修复""写出 SQL"），并给清：要实现的功能、
                  输入输出或期望行为、约束（时间/空间/边界条件），以及"写到什么程度"
                  （完整可运行的最小实现 vs 只写核心函数/关键片段）；
                - 学习者提交的是代码/配置/命令本身，评分点按"代码是否正确实现了题干要求的行为"
                  逐条设置、可客观核验（正确性、关键边界；复杂度只有在题干明确要求时才考）；
                - 纯概念（定义、原理、权衡、对比、原因）仍出解释题，不要硬套代码。
        ...
        String user = String.format("""
                ...
                - 【动手实现优先】可实现类概念（算法/数据结构/并发/数据库/网络/脚本/配置/框架机制）优先出
                   "动手实现"题：让学习者写出代码 / SQL / 命令 / 配置，并说清功能、输入输出或期望行为与约束；
                   评分点按"代码是否正确实现了题干要求的行为"设置（正确性、关键边界，复杂度仅在题干明确要求时考）；
                   纯概念题（定义/原理/权衡/对比/原因）才用解释型。
