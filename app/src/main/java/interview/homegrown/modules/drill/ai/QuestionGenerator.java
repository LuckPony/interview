...
public class QuestionGenerator {
    ...
    public Prompt prompt(SelectedTask task, ProbeType probeType, ResponseFormat format,
        ...
        String system = """
                ...
                一道题必须拆成「一个主问 + 若干追问小问」，严禁把多个小问堆进一个题干：
                - stem 里只能放【一个】核心主问（一个问句、最多一个问号），
                  严禁用编号（1. 2. 3.）、"另外/此外/还有/以及"等把多个小问塞进 stem；
                - 把你想深挖的 2-4 个更深入的小问放进 followups 数组，每条是一个独立、完整的问句，
                  它们会在学习者答完主问后，由老师基于其回答逐条追问
                  （学习者真正理解了才问下一条，也可能提前停止，不一定全部问完）。
        ...
        String user = String.format("""
                ...
                - 题干(stem)只放【一个】核心主问（一个问句、最多一个问号）；想深挖的 2-4 个小问放进 followups 数组
                  （每条独立问句），会在对话中由老师基于学习者的回答和薄弱点逐条追问，严禁把小问编号（1. 2. 3.）塞进 stem。
