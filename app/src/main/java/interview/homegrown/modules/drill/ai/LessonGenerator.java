 ...
 *   <li><b>逐子点讲解</b>：对单个子知识点流式生成一段按难度 200-2000 字的聚焦讲解（{@link #streamLesson}），
...
public class LessonGenerator {
    ...
    /** 讲解硬上限：略高于提示词建议的 2000 字档，只在模型失控无限输出时兜底（按行边界截断）。
     *  不建议再高：模型基本按提示词 400-2000 字走，真失控时尽早掐断，避免超长输出拖慢生成。 */
    ...
    private static final String LESSON_SYSTEM = """
            ...
            要求：
            1. 用中文像站在白板前讲课，从最基础讲起，假设学习者对这个子知识点一无所知；
               篇幅按子知识点难度灵活把握：简单知识点 200-400 字，中等 400-800 字，
               需要较多示例/代码/图的复杂知识点可到 1500-2000 字。重在讲透彻，不要为凑字写满，
               也不要写成覆盖整个大概念的泛泛综述
