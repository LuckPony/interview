...
/**
 * 面试出题服务
 * 根据 Skill 方向 + 难度 + 简历内容，调用 LLM 生成题目列表
 */
@Service
public class InterviewQuestionService {
     ...
     * @param resumeText     简历文本（可为空，为空则只按方向出题）
     * @param provider       LLM Provider
     */

    //配置Ai
    public InterviewQuestionResult generateQueations(String skillId,
                                                     InterviewDifficulty difficulty,
                                                     int questionCount,
                                                     String resumeText,
                                                     String provider){

        String direction = skillService.getSkillDescription(skillId);
        ...
        log.info("出题完成: skillId={}, difficulty={}, 题目数={}",
                skillId, difficulty, result.questions().size());
        return result;
    }
